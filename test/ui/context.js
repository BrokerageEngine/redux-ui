...
describe("UI state context", () => {
  ...
  describe("single-level nested ui component tree", () => {
    ...
    class Parent extends Component {
      updateContext(to = "parent") {
        this.props.updateUI("name", to);
      }
      render() {
    ...
    class Child extends Component {
      updateParentContext(to = "parent") {
        this.props.updateUI("name", to);
      }
      updateName() {
        this.props.updateUI("name", "bar");
      }
      updateOwnState() {
        this.props.updateUI("child", "foo");
      }
      render() {
        return <p>Nested</p>;
      }
    ...
    it("child component inherits parent context", () => {
      ...
      assert(
        shallowEqual(c.props.ui, uiState),
        "child inherits parent UI state context"
      );
    });
    ...
    it("parent component only gets its own context", () => {
      ...
      assert(
        shallowEqual(p.props.ui, uiState),
        "parent only has its own context and not childrens"
      );
    });
    ...
    it("child merges parent UI context with its own UI context", () => {
      ...
      assert(
        shallowEqual(child.props.ui, expectedState),
        "child merges context"
      );
    });
    ...
    it("child updates own context separately from parent", () => {
      ...
      assert(
        parent.props.ui.child === undefined,
        "parent has no child state in its context"
      );
    });
